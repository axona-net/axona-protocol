 ...
 */
export async function createReporter(onStatus = () => {}) {
...
/** One-shot: connect, publish, disconnect. Used for a single (non-continuous) run. */
export async function reportToAxona(result, onStatus = () => {}) {
  const r = await createReporter(onStatus);
  try {
    onStatus('publishing result to Axona…');
    const { msgId } = await r.publish(result);
    await new Promise((res) => setTimeout(res, 2500));      // let it propagate to roots
