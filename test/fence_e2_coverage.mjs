...
//   (3) asserts the 39 IMMUTABLE site identities are distinct (a real site set);
//   (4) OWNERSHIP: every mapped (boundary, wire, transportKind) → rowType is
//       actually declared by that boundary registry;
//   (5) INJECTION: the 39 mapped (boundary, rowType) are distinct;
...
check('C2. exactly 39 migration-target source sites in the artifact (38 E0 originals + dht:presence)', sites.length === 39, `\n   got ${sites.length}`);
...
// ── (3) the 39 IMMUTABLE call-site identities are distinct, and each maps to
...
check('C4. the 39 immutable identities (file:line ‖ wire ‖ transportKind ‖ receiver — LOCATOR-led) are DISTINCT',
  new Set(sites.map((s) => s.identity)).size === 39,
...
// ── (5) INJECTION: the 39 mapped (boundary, rowType) are distinct — no two sites share a row ──
const rowKeys = sites.map((s) => `${s.boundary}|${s.rowType}`);
check('C6. INJECTION: the 39 mapped (boundary, rowType) targets are DISTINCT — no two source sites claim the same row',
  new Set(rowKeys).size === 39,
